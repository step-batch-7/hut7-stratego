...
const validatePiecesCount = function(pieceSetup, individualPieceInfo) {
  const expectedPieces = Object.keys(pieceSetup);
  const actualPieces = Object.keys(individualPieceInfo);
  return actualPieces.every(piece => {
    return (
      expectedPieces.includes(piece) &&
      individualPieceInfo[piece] === pieceSetup[piece]
    );
  });
};
...
const validateSetupData = function(pieceInfo) {
  ...
  return validatePiecesCount(pieceSetup, individualPieceInfo);
};
...
const setupData = function(req, res) {
  ...
  if (validateSetupData(piecesInfo)) {
    games.arrangeBattleField(gameId, { unit, piecesInfo });
