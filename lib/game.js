...
class Game {
  ...
  getAttackStatus(from, to) {
    ...
    const pieceAttack = defenderRank < attackerRank;
    return this.isSpecificAttack(defenderRank, attackerRank) || pieceAttack
      ? 'won'
      : 'lost';
  }

  isSpecificAttack(defenderRank, attackerRank) {
    const spyMarshalAttack = defenderRank === 10 && attackerRank === 1;
    const bombMinerAttack = defenderRank === 11 && attackerRank === 3;
    return spyMarshalAttack || bombMinerAttack;
  }
