...
const isValidScoutMove = function(currentPosition, targetPosition) {
  const isRowSame = currentPosition[0] === targetPosition[0];
  const isColumnSame = currentPosition[1] === targetPosition[1];
  return isRowSame || isColumnSame;
};
...
class Game {
  ...
  getArmy(unit) {
    ...
    return [...playerArmy, ...opponentArmy];
  }
  ...
  isValidMove(currentPosition, targetPosition) {
    ...
    if (piece && piece.name === 'scout') {
      pieceValidation = isValidScoutMove(currentPosition, targetPosition);
    }
    return (
      this.isValidSourceAndTarget(currentPosition, targetPosition) &&
      pieceValidation
    );
  }
